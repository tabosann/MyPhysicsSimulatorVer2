...
DX12Wrapper::DX12Wrapper(HWND hwnd, int windowWidth, int windowHeight) {
	...
	_initCamera   = _cameraPos = XMFLOAT3(0.f, 0.f, 0.f);
...
DX12Wrapper::CreateSceneDataViewWithCBV() {
	...
	_mapSceneData->proj = XMMatrixPerspectiveFovLH(
		XM_PIDIV2,
		static_cast<float>(swapChainDesc1.Width) / static_cast<float>(swapChainDesc1.Height) ,
		0.1f,
		100.0f
	);
...
ComPtr<ID3D12DescriptorHeap> DX12Wrapper::GetDescHeapForImGui() const { return _descHeapForImGui.Get(); }

void DX12Wrapper::SetBackGroundColor(const XMFLOAT4& bgColor)         { _bgColor = bgColor; }
void DX12Wrapper::SetFieldOfView(float fov)                           { _fov = fov; }
void DX12Wrapper::SetCameraPos(const XMFLOAT3& camera)                { _cameraPos = camera; }
void DX12Wrapper::SetLightDirection(const XMFLOAT4& dir)              { _lightDir = dir; }
